...
void GameControllerClass::gameLoop() {
    ...
    std::vector <ItemClass> upstairsItems = { noteUpA }; //Upstairs items
    ...
    while (true) {
        ...
        else if (command == "INSPECT") //If user wants to pick up item
        {
            ...
            if (currentRoom_temp.getRoomItemByName(command).getName() == command) //Checks if there is an item in the room same as item user wants to inspect
            {
                ...
                if (currentRoom_temp.getRoomItemByName(itemName).getInteraction()->getIsPuzzle() == true)
                {
                    currentRoom_temp.getRoomItemByName(itemName).getInteraction()->runInteraction(userPlayer, galleryKey);
                }
